...
// Local sensor update timer
unsigned long lastLocalUpdateTime = 0;
const unsigned long localUpdateInterval = 100; // 100ms for local sensor updates (10 Hz)
...
<body>
  ...
  <div class="latency-info">
    <div>Player 1: Continuous updates | Player 2: ~13 Hz when connected</div>
...
void loop() {
  ...
  // Continuous local sensor updates (even when Player2 is disconnected)
  if (millis() - lastLocalUpdateTime >= localUpdateInterval) {
    if (millis() - lastBroadcastTime >= broadcastInterval) {
      broadcastLocalData();
      lastBroadcastTime = millis();
    }
    lastLocalUpdateTime = millis();
  }
